...
The goal of this phase is to design and run experiments to evaluate the performance of the algorithms on the benchmarks, and based on the empirical results, formulate
...
especially considering that the framework has not been reviewed by different coders.
...
and to evaluate the algorithms on the selected benchmarks. Experiments are discussed in the order they were conducted and the motivations are given for each of them.
...
two cases. Lastly, an interesting phenomenon can be observed on both problems, for stagnant iterations values of $60$ and $80$ around resolutions of a $100$, the number of iterations
is at its highest. This can also be observed in the results of the previous experiment, when ignoring the lower resolutions, and could possibly be explained by these resolutions being high enough
to allow for setting the parameters to set the parameters to values which are close enough to the optimal ones, but not high enough to make drawing these values as likely as for higher resolutions.
...
Furthermore, the average fitness, iterations and CPU time all appear to grow logarithmically with the maximum number of iterations.
...
\begin{figure}
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    \caption{Performance metrics of the $(1 + 1)$ NA algorithm on the \textit{Half} and \textit{Quarter} benchmarks, with a single neuron, and a maximum iterations stopping criteria, over different resolutions}
...
\begin{figure}
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    \caption{Performance metrics of the $(1 + 1)$ NA algorithm on the \textit{TwoQuarters} benchmark, with a maximum number of iterations criteria, over different maximum iterations}
...
The average fitness is $0.79$ for two neurons, and the number of iterations is $174$, which shows that the algorithm was able to converge to the optimal solution in some cases, but ended up in a local optima
in most cases.
...
The last problem on which the $(1 + 1)$ NA algorithm was tested is the \textit{Proben1 Cancer1} benchmark. This problem differs from the previous ones, in the sense that the data which
...
The BNA algorithm can be applied to binary classification problems. As it is the case for the $(1 + 1)$ NA algorithm, the BNA algorithm was tested on the sphere classification problems,
...
Varying the resolution would simply highlight the same properties as the ones observed for the $(1 + 1)$ NA algorithm. Thus the focus will be on the number of V-neurons and the termination
...
got stuck in local optima, with an average fitness converging to $0.75$ for all three cases. Examples of local optima and optimal solutions for both problems are shown in \Cref{fig:bna_twoquarters_visual}
...
average running time of $1134$ seconds.
...
The comparison is based on the fitness and CPU time metrics, as the number of iterations is not directly comparable between the different algorithms.
...
and the CPU time, with priority given to the fitness. For the NEAT and CMA-ES algorithms, the configurations can be found in \Cref{app:configurations}.
...
As a matter of fact, this first experiment was conducted in \cite{na} but it did not result in an improvement of the performance of the algorithm.
...
Furthermore, in addition to the versatility of the algorithms and their performance, other criteria should be considered when selecting an algorithm. Indeed the ease of use of the algorithm,
...
Altogether, the different experiments did not lead to a single algorithm standing out as superior to the others on all aspects and problems, but rather they highlighted the trade-off between the
...
on all of the tested problems, coupled with its ease of use. The even simpler $(1 + 1)$ NA and BNA algorithms are also good choices for simple binary classification problems, while NEAT,
being particularly versatile and performing well, also appears as a good option, after a careful selection of the parameters, in particular on the control problem,
It also is the only algorithm evolving the network topology among the tested ones, which could be a significant advantage for some problems, with no prior knowledge to guide the choice of
network structures. \Cref{fig:flowchart} shows a flow diagram representing the proposed strategy for the selection of the neuroevolution algorithm given the problem class.
...
allowed for an increase in fitness, as also shown in various plots in \Cref{sec:results}, the initial parameter values seemed to play a major role in the quality of the solution. Indeed, the final
...
They should be set to allow for more than a single species to evolve, while preventing the explosion of the number of species. This could potentially be automated or done manually, as it was the
case in the experiments, by running the algorithm with different similarity threshold while observing the evolution of the number of species throughout the generations, and making
use of prior knowledge on the problem, if available, for setting the similarity weights.
Similarly, the population size should also be carefully selected, lower values result in a faster running but more complex problems require a large population size. However, apart from these parameters and the number
of generations, the other parameters were left untouched from the values given in \cite{neat} and still allowed for good performance on the tested problems.
