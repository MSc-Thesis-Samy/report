...
The cut-off date of 2002 is motivated by the release year of the \cite{neat} paper, proposing the NEAT algorithm, which is the most well-known neuroevolution algorithm
...
$b$ is the bias of the neuron. If $b \geq 0$, the output of the neuron is \textit{True} if the dot product is positive and \textit{False} otherwise. If $b < 0$,
the output of the neuron is \textit{True} if the dot product is negative and \textit{False} otherwise.
...
\lVert \overrightarrow{\varphi} \rVert})$. If $b \geq 0$, the output of the neuron is \textit{True} if $\alpha \leq \theta$ and \textit{False} otherwise. If $b < 0$, the output of the neuron is
\textit{True} if $\pi - \alpha \leq \theta$ and \textit{False} otherwise.
